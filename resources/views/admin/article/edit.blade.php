            ...
            <div class="card-body">
                <form action="{{ route('article.update', $article->id) }}" method="post" enctype="multipart/form-data">
                            ...
                            @foreach ($categories as $item)
                                <option value="{{ $item->id }}"
                                    @foreach ($article->category as $category)
                                        {{ $item->id == $category->id ? ' selected ' : '' }} @endforeach>
                                    {{ $item->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="form-group row mb-4">
                            <div class="col-sm-12 col-md-12">
                                <textarea class="form-control summernote" id="descriiption" name="description"> {{ $article->description }}</textarea>
                            </div>
                        </div>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="meta_word" class="form-control" name="meta_word" rows="4">{{ $article->meta_word }}</textarea>
                    </div>
                    <div class="form-group">
                        <label class="btn btn-success  text-white h6" for="meta_description">Meta Description
                            (Optional)</label>
                        <textarea id="meta_description" class="form-control" name="meta_description" rows="4">{{ $article->meta_description }}</textarea>
                    </div>
                    <button type="submit" class="btn btn-success text-bold h6 my-5">Update Article</button>
